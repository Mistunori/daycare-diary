...
・文末は「〜ました」「〜います」「〜でした」「〜ています」を
  場面に応じて使い分ける（単調にしない）
・同じ単語が近接する文中で不自然に繰り返される場合は言い換える
  NG例：「クレヨンでお絵かきをすると、クレヨンを握ったり…」
  → 「クレヨン」が連続して不自然
  OK例：「クレヨンでお絵かきを始めると、しっかり握って夢中で描いていました」
・ただし「上手に」などの自然な褒め表現まで過剰に避ける必要はない
