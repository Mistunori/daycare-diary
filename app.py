...
また、ルールのある遊びで、忍者探しゲームをします。担任が忍者を忍ばせている間、
目を閉じて「いち、にー、さん…」と数えている姿が可愛いらしい♡
「もういいよー」の掛け声で、色んなところを見渡し次々と忍者を見つけていました！
【文体サンプル②】
２組がレストランごっこに招待してもらったのは「カレー＆シチュー屋さん」🍛
どちらかを選び、さらに好きなトッピングを注文して食べることができ、
子ども達はワクワクした様子でした♪オリーブ組に席まで誘導してもらう場面や、
注文をする場面では少し緊張した姿も見られましたが、料理が届くと笑顔がこぼれ、
和気あいあいとした雰囲気の中で楽しく食べていました☺️
【添削ルール】
・上記サンプルのような温かく躍動感のある文体に整える
・子どもの言葉はそのまま「」で引用して生かす
・絵文字・記号（😄☆♡♪✨🌱など）を文脈に合わせて自然に使う
・「！」で場面の躍動感を表現する
・文末は「〜ました」「〜います」「〜でした」「〜ています」を使い分ける
・「でしょう」は絶対に使わない
・「〜など」「〜姿も」の多用を避ける
・同じ言葉が繰り返されていたら言い換える
・一文が70文字を超える場合は分割する
...
・主語と述語がねじれていないか確認する
・曖昧な表現は具体的な描写に置き換える
・添削後に必ず自分で読み返し、不自然な箇所がないか二重チェックしてから回答する
""",
    "連絡帳": """
...
・絵文字・記号（😄☆♡♪✨など）を文脈に合わせて自然に使ってよい
・敬語を自然に使いながらも堅すぎない親しみやすい表現にする
・「でしょう」は絶対に使わない
・「〜など」「〜姿も」「〜していました」の多用を避ける
・文末表現が単調にならないよう「〜です」「〜ました」「〜ています」を使い分ける
・同じ言葉が繰り返されていたら言い換える
・一文が70文字を超える場合は分割する
・助詞（を・が・は・に）の誤用がないか確認する
・読点（、）の位置が自然か確認する
...
def call_proofread_api(
    ...
    user_content += f"\n【添削対象の文章】\n{text}"
    user_content += (
        "\n\n添削後、必ず以下を自己チェックしてください：\n"
        "①「でしょう」を使っていないか\n"
        "②同じ言葉の繰り返しがないか\n"
        "③助詞の誤用がないか\n"
        "④一文が長すぎないか\n"
        "⑤全体を通して声に出して読んだとき自然に聞こえるか\n"
        "チェック完了後にJSONで回答してください。"
    )
