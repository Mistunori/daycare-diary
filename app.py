...
【文体サンプル①】
「虫を見つけたい！」というリクエストがあったので、わくわく農園に出発！！
季節の歌を口ずさみながら、軽快な足並みで進んでいきます♪農園に着くと
「わーい😄」と一斉に走り出し、てんとう虫やモグラ探しが始まりました☆
担任が忍者を忍ばせている間、目を閉じて「いち、にー、さん…」と数えている姿が可愛いらしい♡
「もういいよー」の掛け声で、色んなところを見渡し次々と忍者を見つけていました！
【文体サンプル②】
２組がレストランごっこに招待してもらったのは「カレー＆シチュー屋さん」🍛
注文をする場面では少し緊張した姿も見られましたが、料理が届くと笑顔がこぼれ、
和気あいあいとした雰囲気の中で楽しく食べていました☺️
「カフェごっこしたい！」という声も聞こえてきました！
進級が近づく中で「自分達もやってみたい」という気持ちが高まっていることを感じます✨️
【添削ルール】
・上記サンプルのような温かく躍動感のある文体に整える
・子どもの言葉はそのまま「」で引用して生かす
・「でしょう」は絶対に使わない
・「〜など」「〜姿も」の多用を避ける
・同じ言葉・表現が繰り返されていたら言い換える
  （特に同一文内で「姿」「子どもたち」などが2回出ないよう注意）
・時制を統一する（過去の出来事は全て過去形で統一）
・「〜という声が聞こえてくると、〜していました」のように
  同時に起きた出来事を条件文（〜と）でつなぐのは不自然なので禁止
  → 「〜という声も聞こえ、〜していました」のように
  　 並列（〜も聞こえ）でつなぐこと
・一文が70文字を超える場合は分割する
・一連の動きや流れを描写している文は、
　文を分割すると意味や臨場感が失われる場合があるため
　むやみに分割しない
　例：「手を振ってくれ、車に乗り出発進行！！」のように
　　　動作がつながっている場合は読点でつないだまま残す
・元の文の「流れ・テンポ・臨場感」を最大限に生かす
・分割が必要な場合も、動作のつながりが
　自然に伝わるよう接続を工夫する
・助詞（を・が・は・に）の誤用がないか確認する
・読点（、）の位置が自然か確認する
・主語と述語がねじれていないか確認する
・曖昧な表現は具体的な描写に置き換える
...
・子どもの言葉や具体的なエピソードを生かす
・敬語を自然に使いながらも堅すぎない親しみやすい表現にする
・「でしょう」は絶対に使わない
・「〜など」「〜姿も」「〜していました」の多用を避ける
・同じ言葉・表現が繰り返されていたら言い換える
  （特に同一文内で同じ言葉が2回出ないよう注意）
・時制を統一する
・「〜という声が聞こえてくると、〜していました」のように
  同時に起きた出来事を条件文（〜と）でつなぐのは不自然なので禁止
  → 「〜という声も聞こえ、〜していました」のように
  　 並列（〜も聞こえ）でつなぐこと
・文末は「〜です」「〜ました」「〜ています」を使い分ける
・一文が70文字を超える場合は分割する
・一連の動きや流れを描写している文は、
　文を分割すると意味や臨場感が失われる場合があるため
　むやみに分割しない
