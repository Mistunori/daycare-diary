...
・文末は「〜ました」「〜います」「〜でした」「〜ています」を
  場面に応じて使い分ける（単調にしない）
・ある行為を説明した直後に、その行為に当然含まれる動作を改めて描写しない
  NG例：「クレヨンでお絵かきをすると、クレヨンを握ったり、紙に色をつけたりする姿が見られました」
  → 「クレヨンでお絵かき」と書いた時点で「クレヨンを握る」「紙に色をつける」は自明
  OK例：「クレヨンでお絵かきを始めると、夢中になって何度も描き進めていました」
  → 自明でない具体的な様子（夢中・何度も等）を描写する
・上位概念と下位概念の重複に注意する
  （「食事をすると、スプーンを持って口に運ぶ姿が見られました」等も同様にNG）
・描写する価値があるのは、その場面ならではの具体的な反応・表情・言葉・変化
...
・絵文字は文末か場面の区切りに自然に置く
・1文に絵文字は1つまでにする
・「〜ましたよ」「〜ですよ」など「よ」を付ける文末表現は使用禁止
・一文の文末で「。」と絵文字を同時に使わない
  NG例：「〜ました。😊」「〜でした。✨」
  OK例：「〜ました😊」「〜でした✨」
・文章全体として、「。」で終わる文と絵文字で終わる文が混在するのはOK
  例：「〜しました。〜ていました😊　〜でした。」← これは問題なし
""",
    "連絡帳": """
...
def call_proofread_api(
    ...
    user_content += (
        ...
        "⑧「〜ましたよ」「〜ですよ」を使っていないか\n"
        "⑨文末で「。」と絵文字を同時に使っていないか（「〜ました。😊」はNG、「〜ました😊」はOK）\n"
        "⑩ある行為の説明の直後に、その行為に当然含まれる動作を重複して描写していないか\n"
